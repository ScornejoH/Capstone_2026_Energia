...
/* Forzar las salidas PWM a OFF (medio puente apagado) con digitalWrite directo,
   SIN depender de LEDC ni de las tareas. Llamar como PRIMERA línea de setup()
   para minimizar la ventana en que los gates flotan tras el reset. Además fija
   pull internos en la dirección segura como respaldo ante cualquier alta-Z. */
void pwmForceSafe();
...
/* Activar fault: apaga PWM, latchea faultLatched, guarda el código (CAN_FAULT_*)
   para difusión por CAN y pide escritura a Firebase. */
void tripFault(const char* reason, uint8_t code = 0);
