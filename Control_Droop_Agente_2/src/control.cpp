...
/* ============================================================
   ...
   si P<0. Decimado (4 Hz) + banda muerta + guarda contra huecos largos.
   Acumuladores en double: sin pérdida de precisión en corridas largas.
============================================================ */
...
void taskControl(void* pvParams) {
  ...
  /* ── Calcular ganancias desde los omega/Vdc por defecto, para que el
        control esté bien sintonizado aunque arranque sin config del maestro
        ni WiFi/Firebase. Sin esto el esclavo usaría las ganancias hardcodeadas
        de shared.cpp (kpI=17.59, kiI=11060), ~300× demasiado grandes, y el
        lazo de corriente saturaría → sobrecorriente al levantar el bus. ── */
  recomputeGains();
  ...
  for (;;) {
    ...
    /* ── Soft-start: el límite de corriente sube de 0 a pleno en SOFTSTART_S ──
       Clave aquí: al activar el agente 2 con el bus ya en 5 V (lo sostiene el
       maestro), la base recibida es grande; sin esto, el lazo de corriente
       saltaría a plena corriente y hundiría la fuente. */
    softStart = (softStart < 1.0f) ? (softStart + dt / SOFTSTART_S) : 1.0f;
    float irMaxEff = irMax * softStart;
    float irMinEff = irMin * softStart;

    /* ── Esclavo droop con respaldo autónomo (Opción 3) ──
       Si llega i_ref_v fresca del maestro → lo sigue (base común) y mantiene
       su integrador de voltaje PRECARGADO para conmutar sin salto. Si la
       referencia caduca (maestro desconectado/OFF/en fault) → corre su propio
       PI de voltaje y regula el bus de forma autónoma. */
    float errV = tgtV - vBus;
    float iRefV;
    bool  fresh = ((uint32_t)(millis() - iRefVExtMs) <= IREFV_TIMEOUT_MS);
    if (fresh) {
      iRefV = iRefVExt;                                      // sigue al maestro
      if (kiv > 1e-6f) integV = (iRefV - kpv * errV) / kiv;  // precarga (bumpless al caer el maestro)
    } else {
      integV += errV * dt;                                  // PI de voltaje propio (autónomo)
      iRefV   = kpv * errV + kiv * integV;
      /* Anti-windup (back-calculation): si iRefV satura contra el límite de
         corriente —o contra el clamp reducido del soft-start— se recalcula
         integV para dejarlo justo en el borde. Sin esto el integrador se
         dispara mientras el bus sube y produce sobreimpulso y oscilación de
         ciclo límite (el síntoma de bus oscilante en modo autónomo). */
      if (kiv > 1e-6f) {
    ...
    /* ── Referencia de corriente total = base + droop propio ── */
