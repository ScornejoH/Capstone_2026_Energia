...
static float adsSingleShot(uint8_t ch) {
  ...
  /* Solapar el yield con la conversión: vTaskDelay(1) cede CPU al IDLE
     ...
     no paga 1 ms de yield APARTE → sube de ~400 a ~700 Hz. Luego se sondea el
     bit OS por si faltó (tras el vTaskDelay la conversión ya arrancó, OS=0). */
  vTaskDelay(1);
  uint32_t t0 = micros();
...
/* ============================================================
   UPDATE — leer A2 (bus), A1 (bat), A3 (ACS712) y actualizar shared.
   Bus DC en A2. Para subir el lazo a ~400 Hz: la corriente (ACS, lazo
   interno) se lee CADA ciclo; bus (lazo de voltaje lento) y batería/SoC se
   DECIMAN y se retienen (ZOH) entre lecturas.
     N_BUS=16 → bus ~25 Hz  |  N_BAT=64 → batería ~6 Hz
   Si subes mucho omega_v, baja N_BUS para muestrear el bus más seguido.
============================================================ */
...
bool sensorsUpdate() {
  ...
  static float    lastVa2 = NAN;   // bus (retenido)
