...
static float adsSingleShot(uint8_t ch) {
  ...
  /* Esperar conversión: OS=1 cuando termina. Timeout 10 ms (>> 1.16 ms a 860 SPS).
   * No retorna NAN en fallos I2C individuales — solo en timeout. */
  uint32_t t0 = millis();
  while (!conversionComplete()) {
    if (millis() - t0 > 10) return NAN;
...
/* ============================================================
   ...
   Bus DC conectado en A2 (no en A0).
============================================================ */
bool sensorsUpdate() {
  float va2 = adsSingleShot(2);   // bus DC
  float va1 = adsSingleShot(1);   // batería
  float va3 = adsSingleShot(3);   // ACS712

  if (isnan(va2) || isnan(va1) || isnan(va3)) return false;
